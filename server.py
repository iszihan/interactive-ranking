...
from benchmark_botorch import save_sim_val_npz, get_x_hash, load_sim_val_npz, find_triggers
...
class Engine:
    ...
    def __init__(self, outputs_dir: Path) -> None:
        ...
        #     image_path, gt_img = infer_image_img2img(
        #                     self.component_weights, 
        #                     self.prompt,
        ...
        # else:
        #     self.gt_img = self.preprocess(load_image(self.gt_image_path)).to(device)
    ...
    def next(self, ranking_basenames: List[str]) -> None:
        self.step += 1
        # TODO: Use ranking_basenames to produce next images into self.outputs_dir
...
def next_step(req: NextRequest) -> JSONResponse:
    ...
    images = _list_image_urls()
    return JSONResponse({"images": images, "accepted_ranking": basenames})
